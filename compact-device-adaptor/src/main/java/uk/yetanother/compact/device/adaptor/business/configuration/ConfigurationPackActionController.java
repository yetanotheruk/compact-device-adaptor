...
public class ConfigurationPackActionController {
    ...
    public void buildSchedule() {
        ...
        Set<LocalDateTime> uniqueChangeDates = new HashSet<>();
        for (ConfigurationPack configurationPack : configurationPackCRUDController.getNonExpiredConfigurationPacks()) {
            log.trace(String.format("Scheduling Configuration %s (%s) with the change date of %s.", configurationPack.getExternalReference(), configurationPack.getId(), configurationPack.getValidFrom()));
            uniqueChangeDates.add(configurationPack.getValidFrom());
        }
        log.info("Generated schedule for the given change dates " + uniqueChangeDates);
        scheduleService.scheduleConfigurationPackChanges(uniqueChangeDates);
    }

    public void scheduledChange(List<ConfigurationPack> configurationPacks) {
        executor.submit(new ConfigurationChangeTask(configurationPackMapper.allToDto(configurationPacks), true, configurationPackHandler));
    }

    public void unScheduledChange(UUID id) {
        ...
        if (configurationPack != null) {
            ...
            executor.submit(new ConfigurationChangeTask(Collections.singletonList(configurationPackMapper.toDto(configurationPack)), false, configurationPackHandler));
        } else {
